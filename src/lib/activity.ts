...
export function capsuleWidth(mode: 'island' | 'notch', activity: IslandActivity | null): number {
  if (!activity) return mode === 'notch' ? 280 : 204
  if (activity.type === 'timer') return mode === 'notch' ? 340 : 280
  if (activity.type === 'music') return mode === 'notch' ? 380 : 320
  return mode === 'notch' ? 440 : 380
}
